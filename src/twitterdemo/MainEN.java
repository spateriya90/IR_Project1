...
final int count = 0;
        ...
        SimpleDateFormat dateFormat = new SimpleDateFormat("yyyy-MM-dd HH-mm-ss") ;
    		...
    		File file = new File("English" + dateFormat.format(date) + ".json");
                if(!file.exists()){
    			file.createNewFile();
    		}
 ...
 // TwitterStream twitterStream = new TwitterStreamFactory(cb.build()).getInstance();
                      TwitterStream twitterStream = TwitterStreamBuilderUtil1.getStream();

    StatusListener listener = new StatusListener() {
        int num = 0;
        public void onStatus(Status status) {
           System.out.println("@" + status.getUser().getScreenName() + " - " + status.getLang()
                  +status.getText() + status.getHashtagEntities().toString() + 
                  status.getURLEntities().toString() + status.getGeoLocation().toString() + status.getLang().toString()
                  + status.getPlace().getCountryCode() + status.getPlace().getBoundingBoxCoordinates().toString());
  ...
  //                  + status.isRetweet();
            ...
            /*if(rt == false)
            {
                try {
                    ...
                    Logger.getLogger(MainEN.class.getName()).log(Level.SEVERE, null, ex);
                }
            }*/
                    ;
        }
...
*/
  String[] keywords = {"#trump", "#donaldtrump", "#presidentelect","trump", "Donald Trump", "#notmypresident","#presidentelecttrump", "#recount", "#postcardavalanche", "#NotMyPresident","electoral college", "#president"};
    String[] lang = {"en"};
    fq.track(keywords);
    fq.language(lang);
